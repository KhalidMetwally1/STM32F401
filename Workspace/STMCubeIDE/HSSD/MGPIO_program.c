 ...
 *      => Copy_u8Port  --> GPIO Port 				[ PORTA, PORTB, PORTC ]
 *      => Copy_u8Section  --> GPIO Port Section  	[ LSPART to MSPART ]
 *      => Copy_u32Value --> Pin Value 				[ MGPIO_LOW, MGPIO_HIGH ]
 * Return   : void
 */
void MGPIO_voidSetPortValue		(u8 Copy_u8Port, u8 Copy_u8Section, u32 Copy_u32Value)
{
	if (Copy_u8Section == LSPART)
	{
		switch(Copy_u8Port)
	...
	else if (Copy_u8Section == MSPART)
	{
		...
		case PORTA:
			...
			GPIOA->ODR |= Copy_u32Value << 8;
			break;
		case PORTB:
			GPIOB->ODR &= (0xFFFF00FF);
			GPIOB->ODR |= Copy_u32Value << 8;
			break;
		case PORTC:
			GPIOC->ODR &= (0xFFFF00FF);
			GPIOC->ODR |= Copy_u32Value << 8;
			break;
		}
	}
}
